...
void castVote()
{
    ...
    while (fscanf(voteCountTempReadFile, "%d %s %s %s %d\n", &voteCount.candidate.id, voteCount.candidate.name, voteCount.candidate.party, voteCount.candidate.candidancyFrom, &voteCount.count) != EOF)
    {
        ...
        {
            found = true;
            fprintf(tempFile, "%d %s %s %s %d\n", selectedCandidate.id, selectedCandidate.name, selectedCandidate.party, selectedCandidate.candidancyFrom, voteCount.count + 1);
        }
        else
        {
            fprintf(tempFile, "%d %s %s %s %d\n", voteCount.candidate.id, selectedCandidate.name, selectedCandidate.party, selectedCandidate.candidancyFrom, voteCount.count);
        }
    }

    if (!found) {
        fprintf(tempFile, "%d %s %s %s %d\n", selectedCandidate.id, selectedCandidate.name, selectedCandidate.party, selectedCandidate.candidancyFrom, 1);
    }
